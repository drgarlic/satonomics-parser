...
pub fn min_height(states: &mut States, databases: &Databases, datasets: &AllDatasets) -> usize {
    states
        ...
        .and_then(|last_state_date| {
            let min_datasets_height = datasets.get_min_last_height();
            let min_datasets_date = datasets.get_min_last_date();

            if min_datasets_date.map_or(true, |min_datasets_date| min_datasets_date < *last_state_date) {
                return None;
            }

            datasets.get_date_to_last_height().get(&last_state_date.to_string()).and_then(|state_last_height| {
