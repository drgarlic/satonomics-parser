...
pub fn compute_height_to_coinblocks_stored_map(
    db: &BitcoinDB,
    block_count: usize,
) -> color_eyre::Result<HashMap<String, f64>> {
    println!("Computing height_to_coinblocks_stored...");
    let time = Instant::now();

    let path = Path::new(OUTPUTS_FOLDER_RAW_PATH).join("height_to_coinblocks_stored.json");
    ...
    let mut txid_to_height: FxHashMap<Txid, usize> = FxHashMap::default();
    ...
    db.iter_block::<FBlock>(start, block_count)
        ...
        .try_for_each(|(index, block)| -> color_eyre::Result<()> {
            ...
            let coinblocks_stored: f64 = block
                .txdata
                .iter()
                .map(|tx| {
                    tx.output
            ...
            map.insert(height.to_string(), coinblocks_stored);
