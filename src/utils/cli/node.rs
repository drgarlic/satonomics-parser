...
pub fn wait_node_sync() -> color_eyre::Result<()> {
    while !check_if_node_fully_synced()? {
        sleep(Duration::from_secs(30))
    }
...
pub fn wait_for_new_block(last_block_height: usize) -> color_eyre::Result<()> {
    while get_blockchain_info()?.headers as usize == last_block_height {
        sleep(Duration::from_secs(30))
    }
...
fn check_if_node_fully_synced() -> color_eyre::Result<bool> {
    let BlockchainInfo { blocks, headers } = get_blockchain_info()?;
