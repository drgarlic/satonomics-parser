...
use crate::utils::{Json, IMPORTS_FOLDER_PATH};

struct Binance;

impl Binance {
    pub fn read_har_file() -> color_eyre::Result<HashMap<u32, f32>> {
        let path_binance_har = Path::new(IMPORTS_FOLDER_PATH).join("binance.har");

        let json: HashMap<String, Value> = Json::import_map(path_binance_har);
